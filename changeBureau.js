function changeBureau(input){
    ...
    const chinajenCount = Number(input[1]);
    const changeTaxCommision = Number(input[2]);

    const bitcoinPrice = 1168;
    const chinaJenPrice = 0.15;
    const dollarPrice = 1.76;
    const euroPrice = 1.95;

    const total = ((bitcoinCount * bitcoinPrice) + ((chinajenCount * chinaJenPrice) * dollarPrice)) / 1.95;
    const totalAfterCommision = total - total * changeTaxCommision / 100 
...
//  Той иска да обмени парите си в евро за екскурзията. Напишете програма,

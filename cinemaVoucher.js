function cinemaVoucher(input){
    let index = 0;
    const voucerValue = Number(input[index++]);
    let typeOfGoods = input[index++];
    let currentGoods = input[index++];
    let priceTicket = 0;

        while(currentGoods !== "End"){
            if(typeOfGood.length > 8){
                priceTicket = currentGoods(input[0])
            }
        }
...
//  от които често печели ваучери за кино. Вашата задача е да напишете програма,
//   която да изчислява колко покупки от киното може да си купи Любо със спечеленият ваучер.

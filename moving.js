function moving(input) {
  let index = 0;
  const depthEmptySpace = Number(input[index++]);
  const lengthEmptySpace = Number(input[index++]);
  const hightEmptySpace = Number(input[index++]);
  const emptySpace = depthEmptySpace * lengthEmptySpace * hightEmptySpace;
  let leftSpace = emptySpace;
  let command = "";
  while (command !== "Done") {
    let currentBoxes = Number(command);
    leftSpace -= currentBoxes;
    if (leftSpace <= 0) {
      console.log(
        `No more free space! You need ${Math.abs(leftSpace)} Cubic meters more.`
      );
      break;
    }
    command = input[index++];

function catWalking(input){
    const minutesWalkinPerDay = Number(input[0]);
    const wlakingCountDayly =Number(input[1]);
    const calIntakePerDay = Number(input[2]);
    const calBurnedPerMinute = 5;
    const totalBurnedCal = calBurnedPerMinute * minutesWalkinPerDay * wlakingCountDayly
    const intakeCalDifference = calIntakePerDay * 0.5

    if (totalBurnedCal >= intakeCalDifference){
